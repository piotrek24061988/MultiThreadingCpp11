    ...
    void push(T new_value) {
        ...
        while(1) {
            ...
            if(old_tail.ptr->data.compare_exchange_strong(old_data, new_data.get())) {
                ...
                //Other thread help us to set next ptr
                if(!old_tail.ptr->next.compare_exchange_strong(old_next, new_next)) {
                    delete new_next.ptr;//So we can dealocete new node aloceted by our thread
                    new_next = old_next;//and use node alocated by supporting thread.
                }
